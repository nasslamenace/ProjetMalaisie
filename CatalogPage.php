...
try
{
	...
	$query = 'SELECT * FROM instruments WHERE type= :type LIMIT :limite OFFSET :start ';
	...
	$query->bindValue('type', $_GET['type'], PDO::PARAM_STR );
	...
	$queryIdMax = 'SELECT * FROM instruments WHERE type= :type ORDER BY id DESC LIMIT 0, 1 ';

	$queryIdMax = $bdd->prepare($queryIdMax);

	$queryIdMax->execute(array('type' => $_GET['type']));
...
<html lang="en" dir="ltr">
  <head>
	...
	<?php
    		if($page > 1){
	    		echo '<a class="previousPage" href="?page='. ($page - 1) . "&type=" . $_GET['type']  . '"><------------previous page</a>';
    		}
	    	if($count == $limit){
	   			echo '<a class="nextPage" href="?page=' . ($page + 1). "&type=" . $_GET['type']  . '">next page ----------></a>';
	    	}
		?>
    </body>
</html>